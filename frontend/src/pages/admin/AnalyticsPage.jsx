...
export default function AnalyticsPage() {
  const summaryQ = useQuery({ queryKey: ["analytics-summary"], queryFn: fetchAnalyticsSummary, staleTime: 30_000 });
  const peaksQ = useQuery({ queryKey: ["analytics-peaks"], queryFn: fetchPeakHours, staleTime: 60_000 });
  const durQ = useQuery({ queryKey: ["analytics-duration"], queryFn: fetchAverageDuration, staleTime: 60_000 });
  ...
  return (
    <AppShell title="Analytics" breadcrumbs={["Admin", "Analytics"]}>
      <PageHeader
        title="Analytics"
        subtitle={`Average visit duration: ${durQ.data?.averageVisitMinutes ?? "—"} minutes (n=${durQ.data?.sampleSize ?? 0})`}
      />

      {(summaryQ.isLoading || peaksQ.isLoading || durQ.isLoading) && (
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card title="Visitor trend framing" subtitle="Distinct visitors with at least one check-in during the window.">
          <div className="space-y-2 text-sm">
            {trending.map((t) => (
              <div key={t.label} className="flex items-center justify-between rounded-lg bg-vpms-bg px-3 py-2 ring-1 ring-vpms-border">
                <span className="text-vpms-muted">{t.label}</span>
                <span className="font-bold text-vpms-text">{t.value ?? "—"}</span>
              </div>
            ))}
          </div>
        </Card>
          ...
          <div className="h-56 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={peaksQ.data || []}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="count" stroke="rgb(var(--vpms-accent))" fill="rgba(16, 185, 129, 0.25)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
